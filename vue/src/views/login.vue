      ...
      <div class="main">
        ...
        <Form ref="loginform" :rules="rules" :model="loginModel">
          ...
          <FormItem prop="password">
            <div class="ivu-input-wrapper ivu-input-wrapper-large ivu-input-type">
              <i class="ivu-icon ivu-input-icon ivu-input-icon-normal iconfont" style="left:0">&#xe608;</i>
              <input v-on:keyup.enter="login" v-model="loginModel.password" autocomplete="off" spellcheck="false" type="password" :placeholder="L('PasswordPlaceholder')" class="ivu-input ivu-input-large" style="padding-left:32px;padding-right:0">
            </div>
          </FormItem>
        </Form>
        <div>
...
export default class Login extends AbpBase {
  ...
  async login(){
    (this.$refs.loginform as any).validate(async (valid:boolean)=>{
       if(valid){
          ...
          // console.log(this.loginModel.rememberMe)
          if (this.loginModel.rememberMe ==true) {
            // localStorage.setItem('username', this.loginModel.userNameOrEmailAddress)   JSON.parse(sessionStorage.getItem('userInfo'))
            localStorage.setItem('username', this.loginModel.userNameOrEmailAddress)
            localStorage.setItem('password', this.loginModel.password)
            localStorage.setItem('rememberMe', JSON.stringify(this.loginModel.rememberMe))
          ...
          // localStorage.setItem('rememberMe',this.loginModel.rememberMe?'1':'0');
          // console.log('this.loginModel.rememberMe')
        location.reload();
       }
    });      
  ...
  created(){
    // 记住账户和密码功能
    this.loginModel.userNameOrEmailAddress = localStorage.getItem('username')
    this.loginModel.password = localStorage.getItem('password')
    this.loginModel.rememberMe = JSON.parse(localStorage.getItem('rememberMe'))
    console.log(localStorage)
  }
}
</script>
<style scoped>  
  ...
  @media (min-width: 768px){.container{
    background-image: url("../images/NB-a8.jpg");

                ...
                <Row :gutter="16">
                    ...
                    <i-col span="8">
                        <FormItem :label="L('阶梯数')">
                            <i-select  @on-change="handleOrder" v-model="money.p_PriceLen" placeholder="请选择">
                                <i-option
...
<script lang="ts">
    ...
    @Component
    export default class EditMoney extends AbpBase{
        ...
        visibleChange(value:boolean){
            if(!value){
                ...
                console.log(this.$store.state.money.editMoney)
                this.money=Util.extend(true,{},this.$store.state.money.editMoney);
                console.log(this.money.p_Name)
            }
        }
        // moneyRule={
        ...
        /* 阶梯数选择 */
        mounted () {
            this.handleOrder(this.money.p_PriceLen)
        }
        handleOrder (val) {
            console.log(val)
            // let f = parseInt(val)
